...
describe('position drawing rendering', () => {
  it('renders the TradingView-style short position zones, labels and four controls', () => {
    ...
    expect(markup).toContain('fill="rgba(242,54,69,.2)"')
    expect(markup).toContain('data-testid="position-lower-zone"')
    expect(markup).toContain('fill="rgba(8,153,129,.2)"')
    expect(markup).toContain('data-testid="position-entry-line"')
    expect(markup).toContain('stroke="#787b86"')
    ...
    expect(markup).toContain('data-position-handle="stop"')
    expect(markup).toContain('data-position-handle="target"')
    expect(markup).toContain('font-size="12"')
    expect((markup.match(/data-position-handle=/g) ?? [])).toHaveLength(4)
