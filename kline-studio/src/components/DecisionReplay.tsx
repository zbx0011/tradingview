...
import type {
  DecisionAiWeekSummary, DecisionAttempt, DecisionHistorySort, DecisionPositionMultiplier, DecisionPositionSizingMode, DecisionPracticeMode, DecisionReplayInterval, DecisionReplaySession, DecisionStopLossMode, DecisionSystemTradeSnapshot, DecisionTradeResult,
...
import {
  ...
  formatDecisionDate, formatDecisionDay, formatDecisionWeek, nextDecisionPositionMultiplier, normalizeDecisionPositionMultiplier, pnlForDecisionMode, rewardRiskRatio, sessionResults, symbolPrecision, toggleDecisionHistoryIntervalSelection, toggleDecisionHistorySymbolSelection,
} from '../lib/decisionReplay'
...
function decisionWinStats(results: readonly DecisionTradeResult[], mode: DecisionPositionSizingMode, actor: 'user' | 'system') {
  const eligible = actor === 'system' ? results.filter(decisionResultHasSystemBenchmark) : results
  const wins = eligible.filter((result) => decisionResultPnl(result, mode, actor) > 0).length
  return { wins, total: eligible.length, rate: eligible.length > 0 ? wins / eligible.length * 100 : null }
}
...
export function DecisionReplayCenter({ open, availableCount, totalCount, symbolStats, aiWeekSummariesByMode = { 'fixed-risk': [], 'fixed-notional': [] }, availableCandidateKeys = [], seenTradeKeys = [], sessions, activeSessionId, favoriteKeys = [], onToggleFavorite = () => undefined, onClose, onStart, onContinue, onResults, anomalyCount = 0, anomalyUnavailableCount = 0, anomalyLoading = false, onRedoAnomalies }: {
  open: boolean
  availableCount: number
  totalCount: number
  symbolStats: DecisionSymbolStats[]
  aiWeekSummariesByMode?: Record<DecisionPositionSizingMode, readonly DecisionAiWeekSummary[]>
  availableCandidateKeys?: readonly string[]
  seenTradeKeys?: readonly string[]
  sessions: DecisionReplaySession[]
  activeSessionId: string | null
  onClose: () => void
  onStart: (count: number, symbols: SymbolId[], intervals: DecisionReplayInterval[], positionSizingModes: DecisionPositionSizingMode[], practiceMode: DecisionPracticeMode, lossWeekKey: string | null, lossWeekSizingMode: DecisionPositionSizingMode) => void
  onContinue: () => void
  ...
  const [newSessionPreferences, setNewSessionPreferences] = useState(() => loadDecisionReplayCenterPreferences() ?? ({
    ...
    lossWeekSizingMode: 'fixed-risk' as DecisionPositionSizingMode,
  }))
  const { count, selectedSymbols, selectedIntervals, selectedModes, practiceMode, daySequenceScope, selectedLossWeekKey, lossWeekSizingMode } = newSessionPreferences
  ...
  const availableCandidateKeySet = new Set(availableCandidateKeys)
  const seenTradeKeySet = new Set(seenTradeKeys)
  const lossWeekOptions = aiWeekSummariesByMode[lossWeekSizingMode].filter((week) => week.pnlUsd < 0).map((week) => {
    const scopedTrades = week.trades.filter((trade) => (
      trade.replayableAsDay
      && availableCandidateKeySet.has(trade.key)
      && selectedSymbols.includes(trade.symbol)
      && selectedIntervals.includes(trade.interval)
    ))
    return {
      ...week,
      scopedTotal: scopedTrades.length,
      scopedRemaining: scopedTrades.filter((trade) => !seenTradeKeySet.has(trade.key)).length,
    }
  })
  const effectiveLossWeek = lossWeekOptions.find((week) => week.key === selectedLossWeekKey)
    ?? lossWeekOptions.find((week) => week.scopedRemaining > 0)
    ?? lossWeekOptions[0]
    ?? null
  const lossWeekMode = practiceMode === 'day-sequence' && daySequenceScope === 'loss-week'
      ...
      <div className="decision-center-body">
        ...
        <section className="decision-new-session">
          <div className="decision-new-session-head">
            <div><h3>开始新的决策练习</h3><p>{lossWeekMode ? '从AI净亏损交易周中选择一周，再随机抽取该周一个完整交易日逐根练习。' : practiceMode === 'day-sequence' ? '随机选择一个交易日，按当天信号时间顺序逐笔练习；图表保持整日时间轴，不跳到开仓点。' : '从所有可用模拟订单中打乱抽取，已经出现过的交易不会再次抽到。'}</p></div>
          ...
          <div className="decision-practice-mode" role="group" aria-label="选择决策回放模式">
            <button
              ...
              onClick={() => setNewSessionPreferences((current) => ({ ...current, practiceMode: 'random-count', daySequenceScope: 'all-days' }))}
            ><ListOrdered size={18} /><span><b>自定义题目数量</b><small>跨日期随机抽题，题量由你设置</small></span></button>
            <button
              type="button"
              className={practiceMode === 'day-sequence' && daySequenceScope === 'all-days' ? 'active' : ''}
              aria-pressed={practiceMode === 'day-sequence' && daySequenceScope === 'all-days'}
              onClick={() => setNewSessionPreferences((current) => ({ ...current, practiceMode: 'day-sequence', daySequenceScope: 'all-days' }))}
            ><CalendarDays size={18} /><span><b>随机交易日顺序回放</b><small>随机一天，按信号时间从早到晚逐笔练习</small></span></button>
            <button
              type="button"
              className={lossWeekMode ? 'active' : ''}
              aria-pressed={lossWeekMode}
              onClick={() => setNewSessionPreferences((current) => ({
                ...current,
                practiceMode: 'day-sequence',
                daySequenceScope: 'loss-week',
                selectedLossWeekKey: current.selectedLossWeekKey ?? effectiveLossWeek?.key ?? null,
              }))}
            ><BarChart3 size={18} /><span><b>只做AI亏损周</b><small>选择AI周净盈亏为负的一周逐根练习</small></span></button>
          </div>
          {lossWeekMode && <div className="decision-loss-week-filter" aria-label="选择AI亏损交易周">
            <div className="decision-symbol-filter-head"><b>选择AI亏损交易周</b><span>{decisionPositionSizingLabel(lossWeekSizingMode)} · 按交易日边界归周 · 共 {lossWeekOptions.length} 周</span></div>
            <div className="decision-loss-week-sizing" role="group" aria-label="选择AI亏损周统计口径">
              {(['fixed-risk', 'fixed-notional'] as const).map((mode) => <button type="button" className={lossWeekSizingMode === mode ? 'active' : ''} aria-pressed={lossWeekSizingMode === mode} key={mode} onClick={() => setNewSessionPreferences((current) => ({ ...current, lossWeekSizingMode: mode, selectedLossWeekKey: null }))}>{decisionPositionSizingLabel(mode)}</button>)}
            </div>
            {lossWeekOptions.length === 0
              ? <div className="decision-loss-week-empty">当前数据中没有AI净亏损交易周</div>
              : <div className="decision-loss-week-options">{lossWeekOptions.map((week) => {
                const checked = effectiveLossWeek?.key === week.key
                const disabled = week.scopedRemaining === 0
                const winRate = week.total > 0 ? week.wins / week.total * 100 : 0
                return <label key={week.key} className={`decision-loss-week-option${checked ? ' active' : ''}${disabled ? ' is-disabled' : ''}`}>
                  <input type="radio" name="decision-loss-week" checked={checked} disabled={disabled} onChange={() => setNewSessionPreferences((current) => ({ ...current, selectedLossWeekKey: week.key }))} />
                  <span><b>{formatDecisionWeek(week.startTime, week.endTime)} 交易周</b><small>AI {formatDecisionPnl(week.pnlUsd)} · {week.total} 笔 · 胜率 {winRate.toFixed(1)}%</small></span>
                  <em>{week.scopedRemaining} 笔可练{week.scopedTotal !== week.total ? ` / 当前筛选 ${week.scopedTotal} 笔` : ''}</em>
                </label>
              })}</div>}
          </div>}
          ...
          {practiceMode === 'random-count'
            ...
            : <div className="decision-day-mode-note"><CalendarDays size={17} /><span><b>题数按当天实际交易</b><small>{lossWeekMode ? '仅从所选AI亏损周抽取完整交易日；' : ''}同一标的、同一周期按信号时间顺序练习；有休市的品种从06:00开盘 K 线开始</small></span></div>}
          ...
          <button className="decision-primary" disabled={effectiveAvailableCount === 0 || selectedSymbols.length === 0 || selectedIntervals.length === 0 || selectedModes.length === 0 || (lossWeekMode && (!effectiveLossWeek || effectiveLossWeek.scopedRemaining === 0))} onClick={() => onStart(boundedCount, selectedSymbols, selectedIntervals, selectedModes, practiceMode, lossWeekMode ? effectiveLossWeek?.key ?? null : null, lossWeekSizingMode)}>{lossWeekMode ? '从所选亏损周随机选一天开始' : practiceMode === 'day-sequence' ? '随机选一天并按顺序开始' : '随机抽取并开始'}</button>
        </section>
            ...
            {result ? <div className="decision-history-card-stats">
              ...
              {result.choice === 'traded' ? <>
                <span>你的 <DecisionModeMoneyStack modes={historyModes} valueFor={(mode) => decisionResultPnl(result, mode, 'user')} /><small>{historyPositionMode === 'fixed-risk'
                  ? `初始止损 ${formatPrice(decisionResultInitialStopLoss(result) ?? 0, candidate.symbol)} · ${decisionResultR(result, 'user').toFixed(2)}R × 100U × ${normalizeDecisionPositionMultiplier(result.positionMultiplier)}倍`
                  : `收益率 ${(decisionResultPnl(result, 'fixed-notional', 'user') / (100 * normalizeDecisionPositionMultiplier(result.positionMultiplier))).toFixed(2)}% × 10,000U × ${normalizeDecisionPositionMultiplier(result.positionMultiplier)}倍`}</small></span>
...
export function DecisionChartStatus({ session, attempt, currentPnlByMode = null, systemTrades = null, systemCurrentPnlByMode = null, systemCurrentPnlLocked = false, positionSizingModes = ['fixed-risk'] }: {
  ...
  const [expandedSummaryState, setExpandedSummaryState] = useState<{ sessionId: string; actor: DecisionChartSummaryActor } | null>(null)
  const expandedSummary = expandedSummaryState?.sessionId === session.id ? expandedSummaryState.actor : null
  const results = sessionResults(session)
  ...
  return <aside ref={statusRef} style={decisionChartStatusStyle(statusPreferences)} className={`decision-chart-status${statusDragging ? ' is-dragging' : ''}`} data-testid="decision-chart-status" aria-label="本场练习盈亏">
    ...
    <div className="decision-chart-status-grid">
      ...
      <button type="button" className={`user-total has-value${expandedSummary === 'user' ? ' is-expanded' : ''}`} data-testid="decision-chart-user-total" aria-expanded={expandedSummary === 'user'} aria-controls="decision-chart-pnl-breakdown" onClick={() => setExpandedSummaryState((current) => current?.sessionId === session.id && current.actor === 'user' ? null : { sessionId: session.id, actor: 'user' })}>
...
export function DecisionReplayPanel({ candidate, latestSignal = null, attempt, ordinal, total, currentClose = null, currentPnlUsd = null, currentPnlByMode = null, positionSizingModes = ['fixed-risk'], favorite = false, independentReview = false, preSignal = false, daySequenceMode = false, canAdvanceTrade = true, onToggleFavorite = () => undefined, onAdvance, onSignalExtreme, onFreePrice, onOpenLong = () => undefined, onOpenShort = () => undefined, onSkip, onManualClose, onCancelPending, onNextTrade, onRestartTrade = () => undefined, onStop }: {
  ...
  const visibleSignal = latestSignal
    ? {
        key: latestSignal.id,
        side: latestSignal.signalSide,
        time: latestSignal.signalTime,
        setup: latestSignal.kind === 'entry' ? latestSignal.trade.entry.setup : latestSignal.trade.exit.setup ?? '反向信号',
        reason: latestSignal.kind === 'entry' ? latestSignal.trade.entry.reason : latestSignal.trade.exit.reason ?? '反向信号触发',
        signalIdx: latestSignal.kind === 'entry' ? latestSignal.trade.entry.signalIdx : latestSignal.trade.exit.signalIdx ?? latestSignal.trade.exit.idx,
      }
    : preSignal ? null : {
        key: candidate.key,
        side: candidate.trade.side,
        time: candidate.trade.entry.signalTime,
        setup: candidate.trade.entry.setup,
        reason: candidate.trade.entry.reason,
        signalIdx: candidate.trade.entry.signalIdx,
      }
  const visibleSignalLong = visibleSignal?.side === 'long'
  ...
  const entryReferenceIndexes = new Set(visibleSignal
    ? extractReasonCandleIndexes(visibleSignal.reason, [visibleSignal.signalIdx])
    : [])
    ...
    <aside ref={panelRef} style={decisionPanelStyle(panelPreferences)} className={`decision-detail-panel${dragging ? ' is-dragging' : ''}${resizing ? ' is-resizing' : ''}`} data-testid="decision-detail-panel">
      <header onPointerDown={handleDragStart} title="拖动移动决策详情框">
        ...
        <div><h2>{preSignal ? `${candidate.symbol} · ${INTERVALS[candidate.interval].label} · 日内逐根回放` : `第 ${ordinal} / ${total} 笔 · ${candidate.symbol} · ${INTERVALS[candidate.interval].label}`}</h2><small>{preSignal
          ...
          : <>{independentReview ? '独立复盘 · 原记录不会被覆盖 · ' : ''}{daySequenceMode ? '最新信号 K' : '信号 K'}：{formatDecisionDate(visibleSignal!.time)}{daySequenceMode ? ` · 当前 K：${formatDecisionDate(attempt.cursorTime)}` : ''} · {attempt.result ? '本笔盈亏已锁定，可继续观看后续 K 线' : '系统原结果将在本笔结束后揭晓'}</>}</small></div>
      ...
      <div ref={detailScrollRef} className="decision-detail-scroll">
        ...
        <section className="decision-reason">
          <h3 className={visibleSignalLong ? 'long' : 'short'}>{visibleSignal!.setup}</h3>
          <p>{highlightDecisionReason(visibleSignal!.reason, entryReferenceIndexes)}</p>
        </section></>}
        {attempt.pendingEntryPrice !== null && <section className="decision-order-state">
          <h3><Target size={17} />你的订单</h3>
          <dl><dt>方向</dt><dd>{userLong ? '做多' : '做空'}</dd><dt>类型</dt><dd>{attempt.entryMode === 'market-close' ? '当前 K 收盘开仓' : orderKindLabel(attempt.orderKind)}</dd><dt>仓位</dt><dd>{normalizeDecisionPositionMultiplier(attempt.positionMultiplier)}×</dd><dt>{attempt.fill ? '开仓价' : '挂单价'}</dt><dd>{formatPrice(attempt.pendingEntryPrice, candidate.symbol)}</dd>{attempt.fill && <><dt>成交时间</dt><dd>{formatDecisionDate(attempt.fill.time)}</dd></>}</dl>
        ...
        {attempt.result && <section className="decision-post-exit-summary">
          ...
          <p>你的盈亏 <DecisionModeMoneyStack modes={positionSizingModes} compact={false} valueFor={(mode) => decisionResultPnl(attempt.result!, mode, 'user')} />{decisionResultHasSystemBenchmark(attempt.result)
            ? <>，V5 系统盈亏 <DecisionModeMoneyStack modes={positionSizingModes} compact={false} valueFor={(mode) => decisionResultPnl(attempt.result!, mode, 'system')} />。</>
            : '；本笔为日内手动续单，没有对应的新增 V5 系统交易。'}</p>
          <p>{exitReasonLabel(attempt.result.userExit.reason, attempt.result.stopLossMode)}</p>
          <small>{daySequenceMode
            ? '按 1 继续逐根观看；按 2 开多；按 3 开空。新开仓会保存为下一笔，不覆盖本笔结果。'
            : '按 1 继续逐根观看后续行情；按 4 进入下一笔；按 5 从信号 K 重新开始本笔。'}</small>
        </section>}
      </div>
...
export function DecisionRiskOverlay({ candidate, side, entryPrice, entryLabel = '挂单', stopLoss, takeProfit, positionMultiplier = 1, onPositionMultiplierChange, initialStopLoss = stopLoss, stopLossMode = 'close', onStopLossMode, currentCandleX = null, currentClose = null, currentPnlUsd = null, currentPnlByMode = null, positionSizingModes = ['fixed-risk'], toPrice, toY, onEntryPrice, onStopLoss, onTakeProfit, onConfirm, onCancel, editable = true, showConfirmControls = editable }: {
  ...
  onPositionMultiplierChange?: (positionMultiplier: DecisionPositionMultiplier) => void
  ...
  const normalizedPositionMultiplier = normalizeDecisionPositionMultiplier(positionMultiplier)
  const amountAt = (exitPrice: number): Record<DecisionPositionSizingMode, number> => ({
    'fixed-risk': pnlForDecisionMode('fixed-risk', positionSide, entryPrice, exitPrice, riskSizingStopLoss, normalizedPositionMultiplier),
    'fixed-notional': pnlForDecisionMode('fixed-notional', positionSide, entryPrice, exitPrice, riskSizingStopLoss, normalizedPositionMultiplier),
  })
...
export function DecisionReviewPanel({ result, positionSizingModes = ['fixed-risk'], onBack }: { result: DecisionTradeResult; positionSizingModes?: readonly DecisionPositionSizingMode[]; onBack: () => void }) {
  ...
  const fixedRiskExplanation = positionSizingModes.includes('fixed-risk') ? `；固定风险按 ${decisionResultR(result, 'user').toFixed(2)}R × 100U × ${positionMultiplier}倍计算` : ''
  return <aside className="decision-review-panel">
    ...
    <div className="decision-review-details">
      ...
      <section><h3>你的执行</h3><p>{result.choice === 'skipped' ? '本次选择不参与。' : result.choice === 'unfilled' ? `${choiceLabel(result)}，截至可用行情结束仍未成交。` : `${choiceLabel(result)}；仓位 ${positionMultiplier}×，开仓 ${formatPrice(result.userEntry?.price ?? 0, result.candidate.symbol)}，初始止损 ${formatPrice(initialStopLoss ?? 0, result.candidate.symbol)}，平仓 ${formatPrice(result.userExit.price, result.candidate.symbol)}${fixedRiskExplanation}，离场方式：${exitReasonLabel(result.userExit.reason, result.stopLossMode)}。`}</p></section>
    </div>
